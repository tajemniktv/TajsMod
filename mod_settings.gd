...
func _add_workspace_settings_section(parent: Control) -> void:
    var WB = preload("res://mods-unpacked/TajemnikTV-TajsModded/extensions/scripts/utilities/workspace_bounds.gd")
    
    parent.add_child(HSeparator.new())
    
    var workspace_header = Label.new()
    workspace_header.text = "Workspace / Board Size"
    workspace_header.add_theme_font_size_override("font_size", 26)
    parent.add_child(workspace_header)
    
    var workspace_container = VBoxContainer.new()
    workspace_container.add_theme_constant_override("separation", 10)
    parent.add_child(workspace_container)
    
    # Warning label (shows when enabled)
    var warning_label = Label.new()
    warning_label.text = "⚠️ Large boards may reduce performance. Nodes placed outside vanilla bounds will be repositioned if mod is disabled."
    ...
    warning_label.visible = config.get_value("expanded_workspace_enabled", false)
    
    # Sub-settings (multiplier slider, etc.)
    var workspace_sub = MarginContainer.new()
    workspace_sub.name = "workspace_sub_settings"
    workspace_sub.add_theme_constant_override("margin_left", 20)
    workspace_sub.visible = config.get_value("expanded_workspace_enabled", false)
    
    var workspace_sub_vbox = VBoxContainer.new()
    workspace_sub_vbox.add_theme_constant_override("separation", 8)
    workspace_sub.add_child(workspace_sub_vbox)
    
    var sub_ref = workspace_sub
    var warning_ref = warning_label
    var cfg = config
    
    # Enable toggle
    _settings_toggles["expanded_workspace_enabled"] = ui.add_toggle(workspace_container, "Expanded Workspace", config.get_value("expanded_workspace_enabled", false), func(v):
        cfg.set_value("expanded_workspace_enabled", v)
        sub_ref.visible = v
        warning_ref.visible = v
        WB.set_enabled(v)
        
        # Refresh current screen to apply new camera limit
        if mod_main.get_tree():
            var main_2d = mod_main.get_tree().root.get_node_or_null("Main/Main2D")
            if main_2d and main_2d.has_method("set_screen"):
                main_2d.set_screen(Globals.cur_screen)
    , "Allow placing nodes beyond vanilla board limits (-5000 to 5000).")
    
    workspace_container.add_child(workspace_sub)
    workspace_container.add_child(warning_label)
    
    # Size info label
    var size_label = Label.new()
    size_label.name = "WorkspaceSizeLabel"
    size_label.add_theme_font_size_override("font_size", 22)
    size_label.add_theme_color_override("font_color", Color(0.6, 0.8, 1.0))
    _update_workspace_size_label(size_label)
    workspace_sub_vbox.add_child(size_label)
    
    var size_label_ref = size_label
    
    # Multiplier slider
    var mult_row = HBoxContainer.new()
    mult_row.add_theme_constant_override("separation", 10)
    workspace_sub_vbox.add_child(mult_row)
    
    var mult_label = Label.new()
    mult_label.text = "Board Multiplier"
    mult_label.add_theme_font_size_override("font_size", 22)
    mult_label.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    mult_row.add_child(mult_label)
    
    var mult_value_label = Label.new()
    mult_value_label.text = "%.1fx" % config.get_value("workspace_multiplier", 1.0)
    mult_value_label.add_theme_font_size_override("font_size", 22)
    mult_row.add_child(mult_value_label)
    
    var mult_slider = HSlider.new()
    mult_slider.min_value = 1.0
    mult_slider.max_value = 10.0
    mult_slider.step = 0.5
    mult_slider.value = config.get_value("workspace_multiplier", 1.0)
    mult_slider.size_flags_horizontal = Control.SIZE_EXPAND_FILL
    mult_slider.custom_minimum_size = Vector2(200, 0)
    mult_slider.focus_mode = Control.FOCUS_NONE
    
    var mult_val_ref = mult_value_label
    
    mult_slider.value_changed.connect(func(v):
        mult_val_ref.text = "%.1fx" % v
        cfg.set_value("workspace_multiplier", v)
        WB.set_multiplier(v)
        _update_workspace_size_label(size_label_ref)
    ...
    workspace_sub_vbox.add_child(mult_slider)
    
    # Quick preset buttons
    var presets_row = HBoxContainer.new()
    presets_row.add_theme_constant_override("separation", 8)
    workspace_sub_vbox.add_child(presets_row)
    
    var presets_label = Label.new()
    presets_label.text = "Quick Presets:"
    presets_label.add_theme_font_size_override("font_size", 20)
    presets_row.add_child(presets_label)
    
    for preset in [1.5, 2.0, 3.0, 5.0]:
        var btn = Button.new()
        btn.text = "%.1fx" % preset
        btn.custom_minimum_size = Vector2(60, 35)
        btn.pressed.connect(func():
            mult_slider.value = preset
        )
        presets_row.add_child(btn)


func _update_workspace_size_label(label: Label) -> void:
